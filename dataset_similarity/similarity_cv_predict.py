...
# def get_neighborhood_list(loader, r, n_rand):
#     x_neighborhood_list = []
#     for sample, y in loader:
#         if args.r != 0:
#             tmp = [torch.clamp(sample + r * torch.rand(sample.shape), 0, 1) for i in range(n_rand)]
#             x_neighborhood_list.append(torch.cat(tmp, dim=0))
#         else:
#             x_neighborhood_list.append(sample)
#     return x_neighborhood_list
